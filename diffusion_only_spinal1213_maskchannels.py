...
class Trainer(object):
    ...
    def get_data_from_batch(self, batch_data:torch.Tensor):
        ...
        random_tensor = torch.randint(low=0, high=7, size=(batch_size, ))
        ...
        random_mask = mask_tensor[random_tensor].unsqueeze(dim=-1).unsqueeze(dim=-1)
        return ((batch_data[:,:3,:,:]*random_mask).to(self.device), batch_data[:,3:,:,:].to(self.device))
